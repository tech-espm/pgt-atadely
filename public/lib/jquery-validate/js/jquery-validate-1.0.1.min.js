...
(function () {
	var expDinheiro = /^(\d+|\d+,\d{1,2})$/;
	var expHorario = /^([01]\d|2[0-3]):[0-5]\d$/;
	var urlRegExp = /[^a-z0-9_\-]/gi;
	var aspectRatioRegExp = /^\d+:\d+$/;

	$.validator.addMethod("dinheiro", function (value) {
		return (!value || expDinheiro.test(value));
	}, "Por favor, forneça um número válido.");

	$.validator.addMethod("eventoHorario", function (value) {
		return (!value || expHorario.test(value));
	}, "O horário deve estar entre 00:00 e 23:59.");

	$.validator.addMethod("eventoUrl", function (value) {
		return (!value || !urlRegExp.test(value));
	}, "A URL deve conter apenas letras, números, e os caracteres \"-\" e \"_\".");

	$.validator.addMethod("eventoAspectRatio", function (value) {
		return (!value || aspectRatioRegExp.test(value));
	}, "O aspect ratio deve ter o formato \"999:999\" (16:9, 1:1, 3:4...)");
